...
function getImages($imgId) {
    $db = phpmotorsConnect();
    $sql = 'SELECT imgId, imgPath, imgName, imgDate, inventory.invId, invMake, invModel FROM images JOIN inventory ON images.invId = inventory.invId WHERE imgId=:imgId';
    $stmt = $db->prepare($sql);
...
//Obtain all thumbnail image path information from the "images" table that are based on the vehicleId.
function getThumbnailbyId($invId) {
    ...
    $stmt->execute();
    $ThumbnailbyId = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $stmt->closeCursor();
